...
public class FilterSystemManager : MonoBehaviour
{
    ...
    public float updateInterval = 0.15f; // 0.1-0.2s as requested
    ...
    {
        if (updateCoroutine == null && isRealTimeUpdateEnabled)
        {
    ...
    {
        updateInterval = Mathf.Clamp(interval, 0.05f, 1.0f); // 限制在合理范围内
    }
